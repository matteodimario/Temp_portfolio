...
def train_epoch(data_loader, model, criterion, optimizer):
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        # 1. Reset gradients to zero (from previous batch updates)
        optimizer.zero_grad()

        # 2. Forward pass: Compute model predictions (logits)
        logits = model(X)
        ...
        loss.backward()  
        # Now, each parameter (like model.conv1.weight) has its .grad populated with the gradient

        # 5. Update model parameters using the optimizer
        optimizer.step()  
        # This applies the computed gradients to update the parameters

        # Accumulate loss for logging purposes
