...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply LayerNorm -> Multi-Head Attention -> Residual Connection
        attn_out = self.multi_head_attention(self.norm1(x)) 
        x = x + attn_out  

        # Apply LayerNorm -> MLP -> Residual Connection
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for sequence in x:
            ...
            for head in range(self.num_heads):
                ...
                scores = torch.matmul(q, k.T) / self.scale_factor  # (n+1, n+1)
                attention_weights = self.softmax(scores)           # (n+1, n+1)
