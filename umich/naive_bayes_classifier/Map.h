...
private:
  ...
  class PairComp {
    ...
    public:
    PairComp() : compare() {}
    bool operator()(const Pair_type& k1, const Pair_type& k2) const {
      return compare(k1.first, k2.first);
    }
  };
...
public:
  ...
  Value_type& operator[](const Key_type& k) {
    auto iterator = tree.find({k, Value_type()});
    if (iterator != tree.end()) {
      return iterator->second;
    }
    auto it = tree.insert({k,Value_type()});
    return it->second;
    assert(false);
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    ...
    // for (auto &kv : tree) {
    //   if (kv.first == val.first) {
    //     auto it = tree.find(val);
    //     return std::make_pair(it, false);
    //   }
     
    // }
    // auto it = tree.insert({val.first, val.second});
    // return std::make_pair(it, true);
    // // else {
