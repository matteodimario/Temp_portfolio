...
String& String::erase(size_t pos, size_t len) {
    ...
    char *pos_ptr = (cstr + pos);
    char *len_ptr = (pos_ptr + len);
    // printf("length: ");
    // printf(len);
    if (len == (size_t) - 1) {
        *(pos_ptr ) = a_null_byte;
    ...
    // std::cout << len_ptr << std::endl;
    size_t i = 0;
    for (i = 0; i <= sz - pos; ++i) {
        // std::cout << cstr;
        if (pos + i <= sz) {
            // std::cout << *(len_ptr + i) << std::endl;
            *(pos_ptr + i) = *(len_ptr + i);
        }
    ...
    *(cstr + sz) = a_null_byte;
    ...
    // TODO: Need to still pass test 8!
...
String& String::insert(size_t pos, const String& str) {
    ...
    for (size_t i = sz; i > 0; --i) {
        *(cstr + pos + str.size() + i) = *(cstr + pos + i);
        // std::cout << cstr << std::endl;
    }
    *(cstr + pos + str.size()) = *(cstr + pos);
...
String& String::replace(size_t pos, size_t len, const String& str) {
    ...
    String cstr_temp = cstr;
    cstr_temp.insert(pos, str);
    
    cstr_temp.erase(pos + str.size(), len);
    
    size_t i = 0;
    for (i = 0; i < cstr_temp.size(); ++i) {
        *(cstr + i) = cstr_temp[i];
    }
    *(cstr + i) = a_null_byte;
